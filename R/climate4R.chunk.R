...
#' @title Apply climate4R function to each loaded set of latitudes.
...
#' @param C4R.FUN.args list of arguments passed to the selected C4R function (the name of the function 
#' is the first element of the list). These arguments are passed to function \link{\code{do.call}}. 
#' See Details.
...
#' Usefull when the amount of data after the C4R function application is large, i.e. similar
#' to the pre-processed data (e.g. when the \link{\code{biasCorrection}} function is applied.)
#' @details In \code{C4R.FUN.args}, for the parameters (of a particular C4R function) where 
#' data (grids) need to be provided, here, a list of 2 arguments are passed: 
#' \code{list(dataset = "", var = "")}.
#' Note that the appropriate libraries need to be loaded before applying this function (loadeR, transformeR,
#' etc.)
#' @return If \code{output.path} is NULL a grid containing all latitudes is returned. If \code{output.path}
#' is provided *.rda objects for each latitude chunk are saved in the spedified path.
#' @family climate4R
#' 
#'
#' @author M. Iturbide
